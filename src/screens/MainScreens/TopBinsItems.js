...
import {
  View, Text, StyleSheet, FlatList, ImageBackground, StatusBar,
  Pressable, Image, TouchableOpacity, Switch, TextInput,
...
// ── Attractive Promotion/Product Card ─────────────────────────
  ...
  return (
    <TouchableOpacity style={styles.productCard} onPress={onPress} activeOpacity={0.92}>
      <View style={styles.imageContainer}>
        <SmartImage uri={imageUri} fallback={PRODUCT_FALLBACK} style={styles.productImage} />
        <View style={styles.imageOverlay} />

        {/* Status badges */}
        {isPromotion && isActive && !isExpired && (
      ...
      <View style={styles.productInfo}>
        ...
        {item.end_date && (
          <Text style={styles.expiryText}>
            {isExpired ? '⚠ Expired' : `Ends ${new Date(item.end_date).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})}`}
          </Text>
        )}
      </View>
    </TouchableOpacity>
...
const TopBinsItems = () => {
 ...
 useFocusEffect(
  useCallback(() => {
    // Always fetch fresh promotions — ignore passed params
    fetchProducts();
    fetchFavorites();
  }, []),
);
...
const fetchProducts = async () => {
  try {
    ...
    list = list.map(promo => {
      ...
      const dist     = store && storeLat && storeLon && !isNaN(storeLat) && userLocation
    ...
    // Step 4: sort by distance if location available
    if (userLocation) {
      list.sort((a, b) => {
    ...
    console.log('=== SAMPLE PROMO ===', JSON.stringify(list[0]));

    // Step 5: fallback to products if no promotions
    if (list.length === 0) {
      console.log('No promotions — falling back to trending products');
      const pRes = await productsAPI.getTrending().catch(() => []);
      list = Array.isArray(pRes) ? pRes : [];
    }

    setAllProducts(list);
    filterProducts(list, showSoldItems, showExpired, searchQuery);
  } catch (err) {
  ...
  const filterProducts = (products, includeSold, includeExpired, query) => {
    let filtered = products;

    if (!includeSold)   filtered = filtered.filter(p => !p.sold);
    if (!includeExpired) filtered = filtered.filter(p => !p.end_date || new Date(p.end_date) >= new Date());

    if (query.trim()) {
      const q = query.toLowerCase();
      filtered = filtered.filter(p =>
        (p.title       || p.name || p.product_name || '').toLowerCase().includes(q) ||
  ...
  const handleSearch = text => {
    setSearchQuery(text);
    filterProducts(allProducts, showSoldItems, showExpired, text);
  };

  const handleToggleSold = () => {
    const next = !showSoldItems;
    setShowSoldItems(next);
    filterProducts(allProducts, next, showExpired, searchQuery);
  };

  const handleToggleExpired = () => {
    const next = !showExpired;
    setShowExpired(next);
    filterProducts(allProducts, showSoldItems, next, searchQuery);
  };
  ...
  const handleToggleFavorite = async productId => {
    ...
    try { await storesAPI.favorite(productId); } catch (e) {}
  };
        ...
        <View style={styles.searchParent}>
          <View style={styles.searchContainer}>
            ...
            {searchQuery.length > 0 ? (
              <TouchableOpacity onPress={() => handleSearch('')} style={styles.iconPad}>
        ...
        <View style={styles.controlsRow}>
          ...
          <View style={styles.togglesRow}>
            <View style={styles.toggleRow}>
              ...
              <Switch trackColor={{false: '#ddd', true: '#56CD54'}} thumbColor="#fff" onValueChange={handleToggleSold} value={showSoldItems} />
            </View>
            <View style={styles.toggleRow}>
              <Text style={styles.toggleLabel}>Expired</Text>
              <Switch trackColor={{false: '#ddd', true: '#E8A020'}} thumbColor="#fff" onValueChange={handleToggleExpired} value={showExpired} />
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  countBadge:  {backgroundColor: '#130160', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4},
  ...
  iconPad:         {padding: 10},
  input:           {flex: 1, fontSize: hp(2), fontFamily: 'Nunito-Regular', color: '#000'},
  ...
  controlsRow:  {flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: '4%', marginBottom: hp(1.5)},
  resultsInfo:  {flexDirection: 'row', alignItems: 'center', gap: 8},
  resultsText:  {fontFamily: 'Nunito-SemiBold', fontSize: hp(1.8), color: '#666'},
  nearbyBadge:  {flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#14BA9C22', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2},
  nearbyBadgeText:{fontFamily: 'Nunito-SemiBold', color: '#14BA9C', fontSize: hp(1.4)},
  togglesRow:   {flexDirection: 'row', gap: 12},
  toggleRow:    {flexDirection: 'row', alignItems: 'center', gap: 4},
  toggleLabel:  {fontFamily: 'Nunito-SemiBold', color: '#333', fontSize: hp(1.6)},
  ...
  emptyContainer:  {flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: hp(10)},
  emptyText:       {fontFamily: 'Nunito-Bold', fontSize: hp(2.5), color: '#666', marginTop: 16},
  emptySubtext:    {fontFamily: 'Nunito-Regular', fontSize: hp(1.8), color: '#999', marginTop: 6},

  // Product Card
  productCard:    {width: wp(44), borderRadius: 16, backgroundColor: '#fff', elevation: 5, shadowColor: '#000', shadowOffset: {width: 0, height: 4}, shadowOpacity: 0.12, shadowRadius: 8, overflow: 'hidden'},
  imageContainer: {position: 'relative'},
  productImage:   {width: '100%', height: hp(15)},
  imageOverlay:   {position: 'absolute', bottom: 0, left: 0, right: 0, height: hp(4), backgroundColor: 'rgba(0,0,0,0.08)'},

  promoBadge:     {position: 'absolute', top: 8, left: 8, backgroundColor: '#130160', borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3, flexDirection: 'row', alignItems: 'center', gap: 3},
  promoBadgeText: {color: '#fff', fontFamily: 'Nunito-Bold', fontSize: hp(1.1)},
  ...
  expiredOverlay: {position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', alignItems: 'center'},
  expiredText:    {color: '#fff', fontFamily: 'Nunito-Bold', fontSize: hp(2.2), letterSpacing: 1},
  ...
  soldBadge:      {backgroundColor: '#fff', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 6},
  soldText:       {color: '#130160', fontFamily: 'Nunito-Bold', fontSize: hp(2)},

  heartBtn: {position: 'absolute', right: 8, top: 8, backgroundColor: '#fff', borderRadius: 16, padding: 5, elevation: 3},

  productInfo:   {padding: 10},
  productName:   {fontFamily: 'Nunito-Bold', color: '#130160', fontSize: hp(1.8), marginBottom: 3, lineHeight: hp(2.4)},
  storeRow:      {flexDirection: 'row', alignItems: 'center', gap: 3, marginBottom: 4},
  storeName:     {fontFamily: 'Nunito-Regular', color: '#14BA9C', fontSize: hp(1.4), flex: 1},
  categoryBadge: {backgroundColor: '#F0F4FF', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, alignSelf: 'flex-start', marginBottom: 6},
  categoryText:  {fontFamily: 'Nunito-SemiBold', color: '#0049AF', fontSize: hp(1.2)},
  priceRow:      {flexDirection: 'row', alignItems: 'center', gap: 6},
  price:         {fontFamily: 'Nunito-Bold', color: '#130160', fontSize: hp(2.1)},
  originalPrice: {fontFamily: 'Nunito-Regular', color: '#bbb', fontSize: hp(1.5), textDecorationLine: 'line-through'},
  expiryText:    {fontFamily: 'Nunito-Regular', color: '#E8A020', fontSize: hp(1.3), marginTop: 4},
});