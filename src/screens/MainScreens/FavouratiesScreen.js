...
const SmartImage = ({uri, fallback, style, resizeMode = 'cover'}) => {
  ...
  return (
    <Image
      ...
      onError={e =>
        console.log('Image load error:', e.nativeEvent.error, 'uri:', uri)
      }
    />
  );
};
...
// ─── Location Tab — shows checked-in stores ────────────────────
const MyItemsScreen = ({loading, checkedInStores, onCheckOut, onCardPress}) => {
  const renderItem = ({item}) => {
    const name = item.store_name || 'Store Name';
    ...
    return (
      <TouchableOpacity
        ...
        {/* ✅ Location pin icon — tap to check out */}
        <TouchableOpacity
          ...
          <View style={styles.heartBg}>
            <Ionicons name="location" size={hp(2.4)} color="#FF3B30" />
          </View>
        </TouchableOpacity>
          ...
          <View style={{flex: 1}}>
            ...
            {/* <Text style={styles.storeDistance}>
              {item.user_latitude && item.user_longitude
                ? `${Number(item.user_latitude).toFixed(2)}°, ${Number(
                    item.user_longitude,
                  ).toFixed(2)}°`
                : 'Location N/A'}
            </Text> */}
  ...
  if (!checkedInStores || checkedInStores.length === 0)
    return (
      <View style={styles.emptyContainer}>
        <Ionicons name="location-outline" size={60} color="#ccc" />
        <Text style={styles.emptyText}>No check-ins yet</Text>
        <Text style={styles.emptySubtext}>
          Visit a store and tap "Check In" to see it here!
        </Text>
      </View>
    );
...
const FavouritesScreen = () => {
  ...
  // ✅ Location tab state — checked-in stores
  const [loadingCheckIns, setLoadingCheckIns] = useState(true);
  const [checkedInStores, setCheckedInStores] = useState([]);

  // Fetch both on focus so data is always fresh
  useFocusEffect(
    useCallback(() => {
      fetchFavorites();
      fetchCheckIns();
  ...
  // ✅ Fetch stores where user has checked in
  const fetchCheckIns = async () => {
    try {
      setLoadingCheckIns(true);
      const response = await storesAPI.getCheckIns();
      let stores = [];
      if (Array.isArray(response)) stores = response;
      else if (Array.isArray(response?.data)) stores = response.data;
  ...
  // ✅ Check out from the Location tab
  const handleCheckOut = (storeId, storeName) => {
    Alert.alert('Check Out', `Check out of ${storeName}?`, [
      {text: 'Cancel', style: 'cancel'},
      {
        text: 'Check Out',
        style: 'destructive',
        onPress: async () => {
          try {
            await storesAPI.checkIn(storeId); // toggles — removes user
            setCheckedInStores(prev => prev.filter(s => s._id !== storeId));
          } catch (err) {
      ...
      <ImageBackground
        ...
        <View style={styles.header}>
          <View style={styles.headerChild}>
            ...
            <Text style={styles.headerText}>
              My Favorite Items & My Locations
            </Text>
          </View>
        </View>
        ...
        <View style={styles.content}>
          {activeTab === 'scan' ? (
            ...
            // ✅ Location tab now shows checked-in stores
            <MyItemsScreen
