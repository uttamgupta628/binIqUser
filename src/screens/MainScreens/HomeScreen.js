import {
  Image, ImageBackground, StatusBar, StyleSheet, Text, TouchableOpacity,
  View, ScrollView, FlatList, Pressable, Dimensions, ActivityIndicator,
  RefreshControl, PermissionsAndroid, Platform, TextInput, Modal,
} from 'react-native';
...
import { storesAPI, productsAPI, userAPI, notificationsAPI, promotionsAPI } from '../../api/apiService';
...
// ── Store Card ─────────────────────────────────────────────────
const StoreCard = ({item, favoriteStores, userLocation, onFavorite, onPress}) => {
  const distance = (() => {
    const storeLat = parseFloat(item.user_latitude ?? item.latitude ?? item.location?.latitude);
    const storeLon = parseFloat(item.user_longitude ?? item.longitude ?? item.location?.longitude);
    if (!userLocation || !storeLat || !storeLon || isNaN(storeLat) || isNaN(storeLon)) return null;
    return getDistanceKm(userLocation.latitude, userLocation.longitude, storeLat, storeLon);
  })();
  ...
  return (
    <Pressable style={styles.storeCard} onPress={onPress}>
      <SmartImage uri={imageUri} fallback={STORE_FALLBACK} style={styles.storeCardImage} />
      <Pressable style={styles.cardHeart} onPress={() => onFavorite(item._id)}>
        <View style={styles.heartBg}>
          <Ionicons name={isFavorited ? 'heart' : 'heart-outline'} size={hp(2.4)} color="#EE2525" />
        </View>
      </Pressable>
...
const ProductCard = ({item, onPress}) => {
  const imageUri = item.images?.[0] || item.image || item.banner_image || item.product_image || null;
  ...
  return (
    <Pressable style={styles.productCard} onPress={onPress}>
      <View style={styles.productImageWrap}>
        <SmartImage uri={imageUri} fallback={PRODUCT_FALLBACK} style={styles.productCardImage} />
        {discountPct && (
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.productPriceRow}>
          ...
          {item.original_price && (
            <Text style={styles.productOriginalPrice}>${item.original_price}</Text>
          )}
        </View>
      </View>
    </Pressable>
...
const FavouriteCard = ({item, onToggle, onPress}) => {
  ...
  return (
    <Pressable style={styles.favCard} onPress={onPress}>
      <SmartImage uri={imageUri} fallback={STORE_FALLBACK} style={styles.favCardImage} />
      <Pressable style={styles.cardHeart} onPress={e => {e.stopPropagation(); onToggle(item._id);}}>
        <View style={styles.heartBg}>
  ...
  <TouchableOpacity style={styles.resellerCard} onPress={onPress}>
    ...
    <View style={styles.resellerInfo}>
      ...
      <View style={styles.resellerMetaRow}>
        ...
        <View style={[styles.resellerMetaBadge, {backgroundColor: '#14BA9C22'}]}>
          <Text style={[styles.resellerMeta, {color: '#14BA9C'}]}>With PDF</Text>
        </View>
      </View>
    </View>
  </TouchableOpacity>
...
// ── Search Modal ───────────────────────────────────────────────
const SearchModal = ({visible, onClose, allStores, allPromotions, navigation}) => {
          ...
          {results.length > 0 ? (
            <FlatList
              ...
              renderItem={({item}) => {
                ...
                return (
                  <TouchableOpacity style={styles.searchResultItem} onPress={() => handleSelect(item)}>
                    <SmartImage
                      uri={imageUri}
                    ...
                    <View style={{flex: 1}}>
                      ...
                      <Text style={styles.searchResultSub} numberOfLines={1}>
                        {isStore
                          ...
                          : `$${item.price || '0'} · ${item.category_id?.name || 'Promotion'}`}
                      </Text>
                    </View>
                    <View style={[styles.searchTypeBadge, {backgroundColor: isStore ? '#0049AF22' : '#14BA9C22'}]}>
                      <Text style={[styles.searchTypeBadgeText, {color: isStore ? '#0049AF' : '#14BA9C'}]}>
                        {isStore ? 'Store' : 'Promo'}
                      </Text>
                    </View>
                  </TouchableOpacity>
            ...
            <View style={styles.searchNoResults}>
              ...
              <Text style={styles.searchNoResultsText}>No results for "{query}"</Text>
            </View>
          ) : (
            <View style={styles.searchNoResults}>
              <Ionicons name="search-outline" size={40} color="#ddd" />
              <Text style={styles.searchNoResultsText}>Search for stores or promotions</Text>
            </View>
          )}
...
const HomeScreen = ({openDrawer}) => {
  ...
  const fetchPromotions = async () => {
    ...
    } catch (e) { console.error('fetchPromotions:', e); }
  };
  ...
  const loadAllStores = async location => {
    try {
      ...
      if (!location) { setAllStores(stores); return; }
      ...
      try {
        const geoRes  = await fetch(
          ...
          {headers: {'User-Agent': 'BinIQ-App'}},
        );
        const geoData = await geoRes.json();
        userCity  = geoData?.address?.city || geoData?.address?.town || geoData?.address?.village || null;
        userState = geoData?.address?.state || null;
      } catch (e) {}

      const withCoords = stores
        .filter(s => s.user_latitude && s.user_longitude)
        .map(s => ({
          ...s,
          _distanceKm: parseFloat(getDistanceKm(latitude, longitude,
            parseFloat(s.user_latitude), parseFloat(s.user_longitude))),
        }))
        .sort((a, b) => a._distanceKm - b._distanceKm);

      const withoutCoords = stores
        .filter(s => !s.user_latitude || !s.user_longitude)
        .map(s => {
          ...
          const uState = (userState || '').toLowerCase();
          let score = 0;
          if (uCity  && sCity  && sCity === uCity)           score = 3;
          else if (uCity  && sAddr && sAddr.includes(uCity)) score = 2;
          else if (uState && sState && sState === uState)    score = 1;
          return {...s, _locationScore: score};
        })
        .sort((a, b) => b._locationScore - a._locationScore);

      setAllStores([...withCoords, ...withoutCoords]);
    } catch (e) {
  ...
  try {
    ...
    const storeLookup = {};
    stores.forEach(s => { if (s.user_id) storeLookup[s.user_id] = s; });
      ...
      <ScrollView
          ...
          <View style={{marginTop: '6%'}}>
            <View style={styles.topBar}>
              ...
              <View style={styles.topBarRight}>
                ...
                <Pressable onPress={() => navigation.navigate('Notifications')} style={styles.notifButton}>
                  <Ionicons name="notifications-outline" size={hp(3.2)} color="#000" />
                  {unreadCount > 0 && (
            ...
            <View style={styles.searchRow}>
              <Pressable style={styles.searchContainer} onPress={() => setShowSearch(true)}>
                ...
                <Text style={styles.searchPlaceholder}>search stores, promotions...</Text>
        ...
        <View style={styles.section}>
          ...
          {!userLocation && (
            <View style={styles.locationWarning}>
              ...
              <Text style={styles.locationWarningText}>Enable location for accurate nearby results</Text>
            </View>
          )}
          <FlatList
        ...
        <View style={styles.section}>
          ...
          <FlatList
            ...
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={styles.hListContent}
            ListEmptyComponent={<HorizontalEmpty message="No items available" />}
          />
        </View>
        ...
        {/* ── RESELLER IQ PORTAL ── */}
        <View style={styles.section}>
          <SectionHeader title="RESELLER IQ PORTAL" onViewAll={() => navigation.navigate('IQPortal')} />
          <FlatList
            data={[
            ...
            renderItem={({item}) => <ResellerCard title={item.title} onPress={item.onPress} />}
...
const styles = StyleSheet.create({
  ...
  topBar:      {width: wp(90), height: hp(5), alignSelf: 'center', marginVertical: '4%', flexDirection: 'row', justifyContent: 'space-between'},
  topBarLeft:  {width: '28%', justifyContent: 'center'},
  topBarRight: {width: '45%', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 8, paddingRight: '4%'},

  locationBar:         {flexDirection: 'row', alignItems: 'center', gap: 4, marginHorizontal: '5%', marginBottom: 6},
  locationText:        {fontFamily: 'Nunito-SemiBold', fontSize: hp(1.6), color: '#555', flex: 1},
  locationWarning:     {flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FFF8E7', borderRadius: 8, padding: 8, marginBottom: 8, borderWidth: 1, borderColor: '#E8A020'},
  locationWarningText: {fontFamily: 'Nunito-Regular', fontSize: hp(1.5), color: '#E8A020'},

  searchRow:        {flexDirection: 'row', alignItems: 'center', marginHorizontal: '3%'},
  searchContainer:  {flex: 1, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 12, marginRight: 10, borderColor: '#99ABC678', height: hp(6), backgroundColor: '#F2F2F2'},
  iconPad:          {padding: 10},
  searchPlaceholder:{flex: 1, fontSize: hp(2), fontFamily: 'Nunito-Regular', color: '#aaa'},
  ...
  horizontalEmpty:     {width: wp(50), height: hp(20), justifyContent: 'center', alignItems: 'center', gap: 8},
  ...
  // Store Card
  storeCard:      {width: wp(47), height: hp(23), marginRight: 12, borderRadius: 12, backgroundColor: '#fff', elevation: 3, shadowColor: '#000', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.08, shadowRadius: 4, overflow: 'hidden'},
  ...
  // Product Card
  productCard:         {width: wp(44), marginRight: 12, borderRadius: 14, backgroundColor: '#fff', elevation: 4, shadowColor: '#000', shadowOffset: {width: 0, height: 3}, shadowOpacity: 0.1, shadowRadius: 6, overflow: 'hidden'},
  productImageWrap:    {position: 'relative'},
  productCardImage:    {width: wp(44), height: hp(14)},
  discountBadge:       {position: 'absolute', top: 8, left: 8, backgroundColor: '#FF4444', borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3},
  discountBadgeText:   {color: '#fff', fontFamily: 'Nunito-Bold', fontSize: hp(1.2)},
  productHeartBtn:     {position: 'absolute', right: 8, top: 8, backgroundColor: '#fff', borderRadius: 16, padding: 5, elevation: 2},
  productInfo:         {padding: 10},
  productName:         {fontFamily: 'Nunito-Bold', color: '#130160', fontSize: hp(1.7), marginBottom: 3, lineHeight: hp(2.3)},
  productStore:        {fontFamily: 'Nunito-Regular', color: '#14BA9C', fontSize: hp(1.4), marginBottom: 4},
  productPriceRow:     {flexDirection: 'row', alignItems: 'center', gap: 6},
  productPrice:        {fontFamily: 'Nunito-Bold', color: '#130160', fontSize: hp(1.9)},
  productOriginalPrice:{fontFamily: 'Nunito-Regular', color: '#bbb', fontSize: hp(1.5), textDecorationLine: 'line-through'},

  // Favourite Card
  favCard:      {width: wp(40), height: hp(22), marginRight: 12, borderRadius: 10, backgroundColor: '#fff', elevation: 3, shadowColor: '#000', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.08, shadowRadius: 4, overflow: 'hidden'},
  ...
  // Reseller Card
  resellerCard:     {width: wp(48), height: hp(22), marginRight: 12, borderRadius: 14, backgroundColor: '#fff', elevation: 4, shadowColor: '#000', shadowOffset: {width: 0, height: 3}, shadowOpacity: 0.1, shadowRadius: 6, overflow: 'hidden'},
  resellerImage:    {width: '100%', height: '60%'},
  resellerOverlay:  {position: 'absolute', top: 0, left: 0, right: 0, height: '60%', backgroundColor: 'rgba(19,1,96,0.15)'},
  resellerInfo:     {padding: '5%'},
  resellerCategory: {fontFamily: 'Nunito-ExtraBold', color: '#0049AF', fontSize: hp(1.4)},
  resellerTitle:    {fontFamily: 'Nunito-Bold', color: '#130160', fontSize: hp(1.9), marginVertical: 3},
  resellerMetaRow:  {flexDirection: 'row', gap: 6},
  resellerMetaBadge:{backgroundColor: '#13016012', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2},
  resellerMeta:     {fontFamily: 'Nunito-SemiBold', color: '#130160', fontSize: hp(1.3)},

  notifButton:    {position: 'relative', padding: 4},
  notifBadge:     {position: 'absolute', top: -2, right: -4, backgroundColor: '#FF3B30', borderRadius: 10, minWidth: 18, height: 18, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 4, borderWidth: 1.5, borderColor: '#fff'},
  notifBadgeText: {color: '#fff', fontSize: 10, fontFamily: 'Nunito-Bold', lineHeight: 13},

  // Search Modal
  searchModalOverlay: {flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-start'},
  searchModalBox:     {backgroundColor: '#fff', borderBottomLeftRadius: 20, borderBottomRightRadius: 20, paddingTop: hp(7), paddingHorizontal: 16, paddingBottom: 16, maxHeight: hp(80)},
  searchModalInputRow:{flexDirection: 'row', alignItems: 'center', backgroundColor: '#F2F2F2', borderRadius: 12, paddingHorizontal: 12, height: hp(6.5), marginBottom: 12},
  searchModalInput:   {flex: 1, fontFamily: 'Nunito-Regular', fontSize: hp(2), color: '#000'},
  searchResultsList:  {maxHeight: hp(50)},
  searchResultItem:   {flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', gap: 10},
  searchResultImage:  {width: wp(12), height: wp(12), borderRadius: 8},
  searchResultTitle:  {fontFamily: 'Nunito-SemiBold', color: '#130160', fontSize: hp(1.8)},
  searchResultSub:    {fontFamily: 'Nunito-Regular', color: '#999', fontSize: hp(1.5), marginTop: 2},
  ...
  searchNoResultsText:{fontFamily: 'Nunito-Regular', color: '#999', fontSize: hp(1.9), marginTop: 10},
  searchCloseBtn:     {alignItems: 'center', paddingVertical: 14, marginTop: 4},
  searchCloseBtnText: {fontFamily: 'Nunito-SemiBold', color: '#130160', fontSize: hp(1.9)},
});