...
const SERP_API_KEY =
  '8d1ab8b3f61cca5975c10f05ed7e8341eebce8bd62ad535945c6095f6903451d';
...
const uploadToCloudinary = async (base64, fileName) => {
  try {
    ...
    if (!res.ok || data.error) {
      console.warn('[Cloudinary] failed:', data.error?.message);
      return null;
    }
...
const buildProductQuery = result => {
  ...
  (result?.localizedObjectAnnotations || [])
    ...
    .forEach(o =>
      candidates.push({text: o.name, score: o.score * 100, source: 'object'}),
    );
  (result?.logoAnnotations || [])
    .filter(l => !isNoise(l.description))
    .slice(0, 1)
    .forEach(l =>
      candidates.push({text: l.description, score: 90, source: 'logo'}),
    );
  ...
  const objCandidate = candidates.find(
    c => c.source === 'object' && c.text !== logoCandidate?.text,
  );
  let query = top.text;
  if (logoCandidate && objCandidate)
    query = `${logoCandidate.text} ${objCandidate.text}`;
  return {
    query: query.trim(),
    bestGuess: bestGuess || top.text,
...
const ImageSearchScreen = () => {
  ...
  const handlePickerResponse = response => {
    try {
      ...
      setTimeout(
        () => runPipeline(asset.base64, asset.uri, asset.fileName),
        100,
      );
    } catch {
      Alert.alert('Error', 'Something went wrong reading the image.');
    }
  };
  ...
  const openCamera = async () => {
    ...
    try {
      launchCamera(
        {...PICKER_OPTIONS, cameraType: 'back'},
  ...
  const openGallery = async () => {
    ...
    try {
      launchImageLibrary(PICKER_OPTIONS, handlePickerResponse);
    } catch {
  ...
  const searchWithQuery = async customQuery => {
    try {
      ...
      const [g, a] = await Promise.all([
        searchGoogleShopping(customQuery),
        searchAmazon(customQuery),
      ]);
      setGoogleCount(g.length);
      setAmazonCount(a.length);
      const merged = mergeResults(g, a);
      if (!merged.length) {
        Alert.alert('No Results', `Nothing found for "${customQuery}".`, [
          {text: 'OK', onPress: () => setScreen('results')},
        ]);
        return;
      }
      setResults(merged);
  ...
  const runPipeline = async (base64, localUri, fileName) => {
    try {
      ...
      let visionResult;
      try {
        visionResult = await analyzeImageWithVision(base64);
      } catch (err) {
      ...
      setAltQueries(
        (allCandidates || [])
          .filter(c => c.text !== q && c.text.length > 2)
          .map(c => c.text)
          .slice(0, 4),
      );
      ...
      if (!merged.length) {
        Alert.alert(
          'No Products Found',
          `No results for "${q}". Try another photo.`,
          [{text: 'Try Again', onPress: () => setScreen('pick')}],
        );
        setScreen('pick');
        return;
  ...
  const saveSingleScan = async (productName, category, cloudinaryUrl) => {
    ...
    if (scanSavedRef.current) {
      console.log(
        '[ImageSearch] scan already saved this run — skipping duplicate',
      );
      return;
    }
          ...
          <TouchableOpacity style={styles.optionCard} onPress={openCamera}>
            ...
            <View style={{flex: 1}}>
              ...
              <Text style={styles.optionSub}>
                Capture a product with your camera
              </Text>
            </View>
        ...
        <View style={styles.tipsBox}>
          ...
          <Text style={styles.tipItem}>
            ...
            <Text style={{fontWeight: '700'}}>
              No screens or TVs in background
            </Text>
          </Text>
          <Text style={styles.tipItem}>
            • Fill the frame with ONLY the product
      ...
      <View style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          {userPhotoLocalUri && (
            <Image
              source={{uri: userPhotoLocalUri}}
              style={styles.loadingPhotoPreview}
              resizeMode="cover"
            />
          )}
          <ActivityIndicator
      ...
      <View style={styles.resultsHeader}>
        ...
        <View
          style={[
            ...
            {backgroundColor: isSaved ? '#2CCCA6' : '#bbb'},
          ]}>
          <MaterialIcons
            name={isSaved ? 'check-circle' : 'schedule'}
            size={12}
            color="#fff"
          />
          <Text style={styles.scanCountText}>
            {isSaved ? '1 scan used' : 'Saving...'}
          </Text>
        </View>
      </View>
        ...
        <View style={styles.altQuerySection}>
          ...
          <ScrollView
            ...
            {altQueries.map((alt, i) => (
              <TouchableOpacity
                key={i}
                style={styles.altQueryChip}
                onPress={() => searchWithQuery(alt)}>
          ...
          return (
            <View key={product.id || index} style={styles.card}>
              <View
                style={[
                  styles.storeTag,
                  {backgroundColor: isAmazon ? '#FFF3E0' : '#E8F5E9'},
                ]}>
                <Text
                  style={[
                    styles.storeTagText,
                    {color: isAmazon ? '#E65100' : '#2E7D32'},
                  ]}>
